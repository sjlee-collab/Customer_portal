...
- `tests/test_email_oracle.py` — 이메일 존재 오라클 차단(B안, 2026-09-18): check-email 스텁(항상 exists:false) · 기존 이메일 신청 → 200 균일 + exists_notified_at 기록(본인 안내 메일 1통) · 15분 쿨다운 · 로그인 미등록/비번오류/비활성 전부 같은 401·문구 · 재설정 요청 미등록/비활성 200(메일 없음). 실행당 ACCOUNT_EXISTS 메일 1통이 싱크로 감(PAR_SAFE 아님)
