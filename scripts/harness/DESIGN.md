...
작성 2026-08-31 (기준 커밋 93066ab, 회귀 10종 231건). **최종 갱신 2026-09-09 — 24종
(회귀 19종 383건 + 자기점검 5종), 2026-09-04 하네스 감사(4축 12건) 전량 해소.** 그간 추가:
스키마 계약·배치 only_test·JWT 실경유·L2 런타임(헤드리스)·이메일 백스톱·flaky 허용목록·
알림 스냅샷·테스트 요청 은닉(관리자만)·거짓통과 봉쇄(양성대조)·SUMMARY 정확 집계,
그리고 **결함 클래스 봉쇄 장치 5종 + 래칫 린트**(§4.1). 상세는 §3~§7.
...
```
┌─ 테스트 계층 ──────────────────────────────────────────────┐
│ tests/ (24종 = 회귀 19종 383건 + 자기점검 5, 2026-09-09)     │
...
│        must_id/Fixtures ─ 생성=정리등록 원자화 (§4.1)      │
│        permission ─ 권한 토글 캡처값 복원 (§4.1)           │
...
## 4.1 결함을 고치지 말고, 결함 클래스를 없앤다 (2026-09-06)

감사를 돌릴 때마다 "또 개선점"이 나왔다. 세어보니 **새 결함이 아니라 같은 클래스가 다시
발견된 것**이었다 — T1이 2건, T2가 5건을 고쳤지만 같은 패턴의 모집단은 76건이었다.
표본을 고치고 모집단을 남기면 감사는 영원히 수렴하지 않는다. 결정적 증거: T2가 거짓통과를
막겠다며 넣은 양성대조 자체가 `len(docs) >= 0`(**항상 참**)이었고, 다음 감사까지 아무도
몰랐다. 손으로 인스턴스를 고치면 이렇게 된다.

그래서 인스턴스를 쫓는 대신 **클래스를 표현 불가능하게 만들거나 기계가 세게** 한다.

| 결함 클래스 | 예전 | 지금 |
|---|---|---|
| 검사 0건이 통과(`0 == 0`) | 러너 skip·픽스처 사망 시 초록 | `report(min_checks=1)` — 0/0은 항상 실패 |
| 빈 목록에 공허한 `all()` | 조회가 오류로 비면 격리 단언 통과 | `t.all_of(name, rows, pred, min_n=1)` |
| `dpost(...)['body']['id']` | 실패 시 KeyError로 죽어 정리·리포트 못 함 | `must_id()` / `Fixtures` — 만든 즉시 정리 등록 |
| 권한을 하드코딩 값으로 복원 | 관리자가 바꾼 운영 설정을 조용히 덮어씀 | `with permission(role, key, enabled)` — 캡처값 복원 |
| `batch()`에 `only_test` 누락 | 운영 전체 스캔·발송·UPDATE (경고는 주석뿐) | 시그니처 기본값 `only_test=True` |

이건 원칙 1(§2)을 하네스 자신에게 적용한 것이다 — **잊었을 때의 기본값이 안전하게.**

**래칫(`test_harness_lint.py`)**: 헬퍼만 만들면 새 테스트가 여전히 옛 패턴을 쓸 수 있으므로,
네 패턴을 파일별로 세어 `tests/lint-baseline.json`보다 **늘면 실패**시킨다. 기존 76건은
한꺼번에 고치지 않는다(그러다 회귀를 깨뜨리는 게 더 위험하다) — 새 코드는 깨끗하고
숫자는 단조 감소하는 쪽을 택했다. 줄인 뒤 `--update-baseline`으로 기준선을 낮춘다.

**헬퍼도 검증한다(`test_itest_helpers.py`)**: T2의 교훈이 "막는다고 주장한 코드가 안 막았다"
이므로, 위 장치들이 실제로 막는지 모의 객체로 확인한다. 두 스위트 모두 AWS를 쓰지 않아
자격증명 없이 어디서나 돌고 운영 데이터를 만들지 않는다.

---

## 5. 확장 규칙 — 새 테스트를 추가할 때

0. **헬퍼를 쓴다(§4.1).** 픽스처는 `Fixtures`, 목록 단언은 `t.all_of`, 권한 토글은
   `with permission(...)`, id 추출은 `must_id()`. 옛 패턴을 쓰면 `test_harness_lint`가
   막는다. 정말 예외가 필요하면 그 줄에 `# lint:allow-<규칙키>`를 달고 이유를 남긴다.
...
### 6.5 drift 감지 — `drift-check.sh`(2026-08-31) + 배포 차단(2026-09-06)
...
**감지는 있었지만 차단이 없었다(2026-09-06 해소).** `deploy-fn.sh`는 drift를 출력한 뒤
`sleep 3` 하고 무조건 배포했다 — 사람이 안 보는 호출(스크립트·세션)에선 게이트가 아예 없고,
사람이 봐도 3초는 판단할 시간이 못 된다. 즉 R4를 막으려고 만든 장치가 R4를 못 막았다.

핵심은 **drift에 두 방향이 있는데 화면상 구분이 안 됐다**는 것이다: ① 레포가 앞섬(정상 배포)
② 배포본이 앞섬(운영 핫픽스를 옛 코드가 덮어 조용한 롤백). 그런데 그 구분값을 스크립트가
**이미 계산해서 출력하고 있었다** — diff의 `-N`(배포본에만 있어 사라질 줄 수). 판단에 안 쓰고
찍기만 했을 뿐이다. 이제 이 값이 0보다 크면 종료코드 3으로 중단하고, 0이면(순수 추가/동일)
아무 방해 없이 진행한다. 의도한 삭제면 `--force`, 판정만 보려면 `--dry-run`.

곁들여 **롤백 아티팩트 보존**: 직전 배포본 zip이 `$TMP`에 있고 `$TMP`는 EXIT 트랩이 지웠다.
스모크가 실패해 스크립트가 죽는 순간 되돌릴 물건이 같이 삭제되던 것(R6 로그인 순단 이력을
감안하면 최악의 타이밍). 배포 전 `~/portal-deploy-backup/`에 복사하고(함수당 10개 유지),
스모크 실패 시 되돌리는 `aws` 명령을 그대로 출력한다.
