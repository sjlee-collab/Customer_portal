...
const MS_FROM          = Deno.env.get("MS_FROM")          ?? "hr@bigxdata.io";
  ...
  const res = await fetch(`https://graph.microsoft.com/v1.0/users/${MS_FROM}/sendMail`, {
    ...
    body: JSON.stringify({
      message: {
        ...
        from: {
          emailAddress: { name: "빅스데이터 고객지원", address: MS_FROM },
        },
  ...
  try {
    ...
    if (body.type === "CONNECTION_TEST") {
      if (!MS_TENANT_ID || !MS_CLIENT_ID || !MS_CLIENT_SECRET)
        return new Response(JSON.stringify({ ok: false, error: "MS_TENANT_ID / MS_CLIENT_ID / MS_CLIENT_SECRET 환경변수 미설정" }), {
    ...
    if (isInsert) {
      jobs.push(sendAndLog(
        requesterEmail,
        `[빅스데이터 고객지원] 요청 접수 확인 - ${record.ticket_number}`,
        customerNewTicketHtml(record, companyName, requesterName),
        record.id, "new_ticket_customer"
      ));
    }
    ...
    if (isInsert && ["contract", "license"].includes(record.category)) {
      ...
      if (managerEmail) jobs.push(sendAndLog(
        managerEmail,
